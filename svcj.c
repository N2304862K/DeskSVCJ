...
// Soft Constraints: Widen bounds for extreme assets (NVDA)
// We rely on the data to constrain the fit, not hard limits.
void check_constraints(SVCJParams* p) {
    if(p->kappa < 0.01) p->kappa = 0.01;   if(p->kappa > 100.0) p->kappa = 100.0;
    if(p->theta < 1e-6) p->theta = 1e-6;   if(p->theta > 20.0) p->theta = 20.0; // Allow extreme variance
    if(p->sigma_v < 0.01) p->sigma_v = 0.01; if(p->sigma_v > 20.0) p->sigma_v = 20.0; // Extreme Vol of Vol
    ...
    // Soft Feller: Only penalize if egregious (Factor 10x) to allow 'breaking' regimes
...
void estimate_initial_params(double* ohlcv, int n, double dt, SVCJParams* p) {
    // Garman-Klass for robust variance seed
    double sum_gk = 0.0;
    for(int i=0; i<n; i++) {
    ...
    p->mu = 0.0; p->theta = rv;
    p->kappa = 2.0; p->sigma_v = sqrt(rv)*0.5; p->rho = -0.5;
    p->lambda_j = 0.5; p->mu_j = 0.0; p->sigma_j = sqrt(rv); 
    check_constraints(p);
}

// Pure Likelihood (No Priors)
double ukf_log_likelihood(double* returns, int n, double dt, SVCJParams* p, double* out_spot_vol, double* out_jump_prob) {
    ...
    for(int t=0; t<n; t++) {
        ...
        // Phenotypic Mixing: 
        // We keep the mixing to prevent numerical NaN, but remove the prior weighting on Theta.
        // We use v_pred itself (no floor) unless it's zero.
        double rob_var = (v_pred < 1e-8) ? 1e-8 : v_pred;
        rob_var *= dt;
        ...
        if(v<1e-8)v=1e-8; if(v>20.0)v=20.0;
        
        if(out_spot_vol) out_spot_vol[t]=sqrt(v_pred);
        if(out_jump_prob) out_jump_prob[t]=post;
        ll += log(den);
    }
    // PURE: No penalty terms added. Returns raw log-likelihood.
    return ll;
}
...
void optimize_svcj(double* ohlcv, int n, double dt, SVCJParams* p, double* out_spot_vol, double* out_jump_prob) {
    ...
    for(int i=0; i<=n_dim; i++) {
        ...
        scores[i] = ukf_log_likelihood(ret, n-1, dt, &t, NULL, NULL);
    }
    ...
    for(int k=0; k<NM_ITER; k++) {
        ...
        double r_score = ukf_log_likelihood(ret, n-1, dt, &rp, NULL, NULL);
        ...
        if(r_score > scores[vs[0]]) {
             ...
             double c_score = ukf_log_likelihood(ret, n-1, dt, &cp, NULL, NULL);
    ...
    if(out_spot_vol) ukf_log_likelihood(ret, n-1, dt, p, out_spot_vol, out_jump_prob);
    free(ret);
}
...
void perform_likelihood_test(double* ohlcv, int len_long, int len_short, double dt, RegimeStats* out) {
    ...
    // Pointer math: offset to start of short window
    int offset = len_long - len_short;
    optimize_svcj(ohlcv + offset*N_COLS, len_short, dt, &p_short, NULL, NULL);
    
    // 3. Calc Likelihoods on Short Data
    double* ret_short = malloc((len_short-1)*sizeof(double));
    compute_log_returns(ohlcv + offset*N_COLS, len_short, ret_short);
    
    // LL(Short | LongParams)
    out->ll_null = ukf_log_likelihood(ret_short, len_short-1, dt, &p_long, NULL, NULL);
    
    // LL(Short | ShortParams)
    out->ll_alt = ukf_log_likelihood(ret_short, len_short-1, dt, &p_short, NULL, NULL);
    ...
    // 5. P-Value (5 Degrees of Freedom)
    out->p_value = chi2_prob(out->statistic, 5);
