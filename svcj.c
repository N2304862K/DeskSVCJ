...
void grid_search_init(double* returns, int n, SVCJParams* p, double theta_anchor, double* out_grid_score) {
    ...
    *p = best_p;
    *out_grid_score = best_score;
}

void optimize_svcj(double* ohlcv, int n, SVCJParams* p, double* out_spot_vol, double* out_jump_prob, int* out_viability) {
    // 1. Garman-Klass Anchor
    double sum_gk = 0.0;
    for(int i=0; i<n; i++) {
    ...
    if(!returns) { *out_viability = -1; return; }
    compute_log_returns(ohlcv, n, returns);
    clean_returns(returns, n_ret);

    // 3. Grid Search Tracer
    double grid_score = -1e15;
    p->theta = theta_anchor;
    grid_search_init(returns, n_ret, p, theta_anchor, &grid_score);
    ...
    for(int restart=0; restart<RESTARTS; restart++) {
        if(restart > 0) {
            p->lambda_j *= (0.5 + ((double)rand()/RAND_MAX)); 
            check_constraints(p);
        }
    ...
    // 5. Viability Tracer Check
    *p = global_best_p;
    
    // Status 1: Improvement over Grid
    // Status 0: Stuck (No improvement)
    // Status -1: Unstable (Boundaries)
    
    *out_viability = 1;
    if(global_best_score <= grid_score + 1e-3) *out_viability = 0; // STUCK
    
    // Hard boundary check
    if(p->theta <= 0.0002 || p->sigma_v >= 4.9 || p->lambda_j >= 290.0) *out_viability = -1; // UNSTABLE
