...
void estimate_initial_params(double* ohlcv, int n, double dt, SVCJParams* p) {
    ...
    p->rho=-0.6; p->lambda_j=0.5; p->mu_j=0; p->sigma_j=sqrt(rv)*2.0;
    check_constraints(p);
}
...
double ukf_log_likelihood(double* ret, int n, double dt, SVCJParams* p, double* sv, double* jp, double proxy) {
    ...
    double pen = (proxy > 0) ? -50.0*pow(log(p->theta)-log(proxy), 2) : 0;
    return ll + pen;
}
...
void optimize_svcj(double* ohlcv, int n, double dt, SVCJParams* p) {
    ...
    for(int i=0; i<=n_dim; i++) {
        ...
        scores[i] = ukf_log_likelihood(ret, n-1, dt, &t, NULL, NULL, proxy);
    }
    ...
    for(int k=0; k<NM_ITER; k++) {
        ...
        double r_score = ukf_log_likelihood(ret, n-1, dt, &rp, NULL, NULL, proxy);
        ...
        if(r_score > scores[vs[0]]) {
             ...
             double c_score = ukf_log_likelihood(ret, n-1, dt, &cp, NULL, NULL, proxy);
...
void run_structural_test(double* ohlcv, int len_long, int len_short, double dt, RegimeMetrics* out) {
    ...
    optimize_svcj(ohlcv, len_long, dt, &p_long);
    out->long_theta = p_long.theta;
    
    // 2. Fit Short (Alt Hypothesis) on slice
    int offset = len_long - len_short;
    optimize_svcj(ohlcv + offset*N_COLS, len_short, dt, &p_short);
    out->short_theta = p_short.theta;
    
    // 3. Calc Raw Likelihoods on Short Data
    double* ret_short = malloc((len_short-1)*sizeof(double));
    compute_log_returns(ohlcv + offset*N_COLS, len_short, ret_short);
    
    // Use 0 as proxy to disable penalty for pure statistical test
    double ll_null = ukf_log_likelihood(ret_short, len_short-1, dt, &p_long, NULL, NULL, 0);
    double ll_alt  = ukf_log_likelihood(ret_short, len_short-1, dt, &p_short, NULL, NULL, 0);
    
    out->ll_ratio_stat = 2.0 * (ll_alt - ll_null);
    if(out->ll_ratio_stat < 0) out->ll_ratio_stat = 0;
    
    out->p_value = 1.0 - chi2_prob(out->ll_ratio_stat, 5); // 5 DOF (kappa,theta,sigv,rho,lam)
