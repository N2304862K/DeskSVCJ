...
void check_constraints(SVCJParams* p) {
    ...
    if(p->sigma_v < 0.05) p->sigma_v=0.05; if(p->sigma_v > 10.0) p->sigma_v=10.0;
...
void estimate_initial_params_ohlcv(double* ohlcv, int n, double dt, SVCJParams* p) {
    ...
    p->mu=0.0; p->theta=rv_annual; p->kappa=4.0; p->sigma_v=sqrt(p->theta);
    p->rho=-0.5; p->lambda_j=0.5; p->mu_j=0.0; p->sigma_j=sqrt(rv_annual)*2.0;
    check_constraints(p);
}

// --- Likelihood Engine ---
// Mode 0: Penalized (Optimizer), Mode 1: Raw (Statistical Test)
double internal_likelihood(double* returns, int n, double dt, SVCJParams* p, double* sv, double* jp, double proxy, int mode) {
    ...
    for(int t=0; t<n; t++) {
        ...
        if(sv) sv[t]=sqrt(v_pred);
        if(jp) jp[t]=post;
        ll += log(den);
    }
    
    if(mode == 0) { // Penalties for Optimizer
        double theta_pen = -50.0 * pow(log(p->theta) - log(proxy), 2);
        return ll + theta_pen;
    }
    return ll; // Raw for Tests
}

double ukf_log_likelihood(double* r, int n, double dt, SVCJParams* p, double* sv, double* jp, double proxy) {
    return internal_likelihood(r, n, dt, p, sv, jp, proxy, 0);
}

// --- Optimization ---
void optimize_svcj(double* ohlcv, int n, double dt, SVCJParams* p, double* sv, double* jp) {
    estimate_initial_params_ohlcv(ohlcv, n, dt, p);
    double proxy = p->theta;
    ...
    for(int i=0; i<=n_dim; i++) {
        ...
        scores[i] = internal_likelihood(ret, n-1, dt, &t, NULL, NULL, proxy, 0);
    }
    ...
    for(int k=0; k<NM_ITER; k++) {
        ...
        double r_score = internal_likelihood(ret, n-1, dt, &rp, NULL, NULL, proxy, 0);
        if(r_score > scores[vs[0]]) {
             ...
             double c_score = internal_likelihood(ret, n-1, dt, &cp, NULL, NULL, proxy, 0);
    ...
    if(sv) internal_likelihood(ret, n-1, dt, p, sv, jp, proxy, 0);
    free(ret);
}
...
void calculate_structural_gradient(double* ohlcv, int len, double dt, GradientStats* out) {
    double wins[8]; double thetas[8]; int cnt=0;
    double curr = (double)len;
    while(curr >= 40 && cnt < 8) { wins[cnt] = curr; curr /= 1.4; cnt++; }
    
    SVCJParams p;
    for(int i=0; i<cnt; i++) {
        int w = (int)wins[i];
        optimize_svcj(ohlcv + (len-w)*N_COLS, w, dt, &p, NULL, NULL);
        thetas[i] = p.theta;
    }
    
    double sx=0, sy=0, sxy=0, sx2=0;
    for(int i=0; i<cnt; i++) {
        double x = log(wins[i]); double y = thetas[i];
        sx+=x; sy+=y; sxy+=x*y; sx2+=x*x;
    }
    out->slope = (cnt*sxy - sx*sy) / (cnt*sx2 - sx*sx);
    out->short_theta = thetas[cnt-1];
    out->long_theta = thetas[0];
    out->curvature = 0.0;
}