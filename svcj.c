...
void estimate_initial_params_smart(double* ohlcv, int n, SVCJParams* p) {
    ...
    double mean=sum_r/(n-1); double var=(sum_r2/(n-1))-mean*mean;
    double kurt = (sum_r4/(n-1))/(var*var);

    p->mu = 0.0; 
    p->theta = rv_annual; if(p->theta < 0.0025) p->theta = 0.0025;
    p->kappa = 4.0; p->sigma_v = sqrt(p->theta) * 2.0; p->rho = -0.6;
    p->lambda_j = 0.1 + (kurt > 3.0 ? (kurt-3.0)*0.5 : 0); 
    p->mu_j = 0.0; p->sigma_j = sqrt(rv_annual/252.0) * 3.0; 
    check_constraints(p);
}
...
void optimize_svcj(double* ohlcv, int n, SVCJParams* p, double* out_spot_vol, double* out_jump_prob) {
    ...
    // Grid Search Init
    double best_grid_score = -1e15;
    SVCJParams best_grid_p = *p;
    double lambdas[] = {0.1, 5.0, 25.0};
    double vol_vols[] = {0.5, 2.0, 6.0};
    
    for(int i=0; i<3; i++) {
        for(int j=0; j<3; j++) {
            SVCJParams temp = *p;
            temp.lambda_j = lambdas[i]; temp.sigma_v = vol_vols[j];
            check_constraints(&temp);
            double s = ukf_log_likelihood(returns, n_ret, &temp, NULL, NULL, theta_anchor);
            if(s > best_grid_score) { best_grid_score = s; best_grid_p = temp; }
        }
    }
    *p = best_grid_p;

    // Nelder-Mead with Restart
    int n_dim = 5;
    double simplex[6][5];
    double scores[6];
    ...
    for(int r=0; r<2; r++) {
        if(r>0) { p->lambda_j *= (0.5 + ((double)rand()/RAND_MAX)); check_constraints(p); }
