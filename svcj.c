...
void initialize_system(double* ohlcv, int n, double dt, int n_particles, EvolvingSystemState* out_state) {
    ...
    double* vol = malloc(n*sizeof(double));
    ...
    // 2. Ensemble Fit (The Gravity Anchor)
    SVCJParams ensemble[5];
    for(int i=0; i<5; i++) {
        int w = natural_window + (i-2)*5;
        int start = n - w;
        optimize_single_window(ret+start, vol+start, w-1, dt, &ensemble[i]);
    }
    
    // 3. Calculate Gravity Distribution
    for(int j=0; j<8; j++) out_state->anchor.mean[j] = 0;
    for(int i=0; i<5; i++) {
        out_state->anchor.mean[0] += ensemble[i].kappa;
        out_state->anchor.mean[1] += ensemble[i].theta;
        out_state->anchor.mean[2] += ensemble[i].sigma_v;
        out_state->anchor.mean[3] += ensemble[i].rho;
        out_state->anchor.mean[4] += ensemble[i].lambda_j;
    }
    for(int j=0; j<5; j++) out_state->anchor.mean[j] /= 5.0;
    
    // 4. Seed Swarm from Gravity
    ...
    for(int i=0; i<n_particles; i++) {
        ...
        out_state->swarm[i].kappa = out_state->anchor.mean[0] + z*0.1;
        out_state->swarm[i].theta = out_state->anchor.mean[1] + z*0.01;
        out_state->swarm[i].sigma_v = out_state->anchor.mean[2] + z*0.05;
        out_state->swarm[i].rho = out_state->anchor.mean[3] + z*0.05;
        out_state->swarm[i].lambda_j = out_state->anchor.mean[4] + z*0.1;
        out_state->swarm[i].mu=0; out_state->swarm[i].mu_j=-0.05; out_state->swarm[i].sigma_j=0.05;
        out_state->swarm[i].weight = 1.0/n_particles;
    }
    
    out_state->n_particles = n_particles;
    out_state->dt = dt;
    double avg_vol=0; for(int i=0;i<n-1; i++) avg_vol+=vol[i];
    out_state->avg_volume = avg_vol/(n-1);
...
void run_system_step(EvolvingSystemState* state, double new_ret, double new_vol, InstantMetrics* out_metrics) {
    ...
    double dt = state->dt;
    double avg_vol = state->avg_volume;
    ...
    for(int i=0; i<n; i++) {
        ...
        check_constraints((SVCJParams*)&new_swarm[i]); // Cast to check
        state->swarm[i] = new_swarm[i]; // Copy back
    }
    free(new_swarm);
    ...
    for(int i=0; i<n; i++) {
        ev_ret += state->swarm[i].weight * state->swarm[i].mu;
        ev_theta += state->swarm[i].weight * state->swarm[i].theta;
        if(state->swarm[i].weight > 1e-9) entropy -= state->swarm[i].weight * log(state->swarm[i].weight);
    }
    ...
    // Escape Velocity
    out_metrics->escape_velocity = fabs(ev_theta - state->anchor.mean[1]);
}