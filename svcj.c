...
void estimate_smart_init(double* ohlcv, int n, SVCJParams* p) {
    // Garman Klass + Simple Grid for first run only
    double sum_gk=0.0;
    for(int i=0; i<n; i++) {
        ...
        sum_gk += 0.5*hl*hl - (2.0*log(2.0)-1.0)*co*co;
    }
    double rv = (sum_gk/n)*252.0;
    p->mu=0; p->theta=rv; p->kappa=4.0; p->sigma_v=sqrt(rv)*2.0; p->rho=-0.6;
    p->lambda_j=0.5; p->mu_j=0; p->sigma_j=sqrt(rv/252.0)*4.0;
    check_constraints(p);
}
...
void optimize_svcj_stateful(double* ohlcv, int n, SVCJParams* p, SVCJParams* prev_p, double* out_spot_vol, double* out_jump_prob) {
    ...
    // 2. Initialization Strategy (Warm Start vs Cold Start)
    double theta_anchor;
    
    if (prev_params != NULL) {
        // Warm Start: Use yesterday's result
        *p = *prev_params;
        theta_anchor = p->theta; // Anchor to previous structural belief
    } else {
        // Cold Start: Estimation + Grid Search
        estimate_smart_init(ohlcv, n, p);
        theta_anchor = p->theta;
        
        // Mini Grid Search for Cold Start
        double best_s = -1e15; SVCJParams best_p = *p;
        double lam[] = {0.1, 5.0}; double sig[] = {0.5, 3.0};
        for(int i=0;i<2;i++) for(int j=0;j<2;j++) {
            SVCJParams t = *p; t.lambda_j=lam[i]; t.sigma_v=sig[j];
            check_constraints(&t);
    ...
    // 3. Optimization (Nelder-Mead with Early Stopping)
    int n_dim = 5;
    double simplex[6][5];
    double scores[6];
    
    // Simplex Setup (Small perturbation for warm start)
    double perturb = (prev_params != NULL) ? 0.05 : 0.20; 

    for(int i=0; i<=n_dim; i++) {
        SVCJParams temp = *p;
        if(i==1) temp.kappa *= (1+perturb); if(i==2) temp.theta *= (1+perturb);
        if(i==3) temp.sigma_v *= (1+perturb); if(i==4) temp.rho -= perturb;
        if(i==5) temp.lambda_j *= (1+perturb);
    ...
    for(int iter=0; iter<MAX_ITER; iter++) {
        ...
        double range = fabs(scores[vs[0]] - scores[vs[5]]);
        if (iter > 20 && range < STOP_TOL) break; // Converged
        
        // ... (Standard NM Reflection/Expansion/Contraction Logic) ...
        // Condensed for brevity, logic identical to previous but inside this loop
