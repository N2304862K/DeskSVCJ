...
double calc_range_likelihood(double h, double l, double v, double dt) {
    ...
    // Widened range gate slightly to tolerate momentum extension
    if (m_dist > 25.0) return 1e-15; 
                  ...
                  SwarmState* out) 
{
    ...
    for(int i=0; i<N_PARTICLES; i++) {
        ...
        // 2. Drift (MOMENTUM CORRECTED)
        // Instead of random walk (0.0 - mu), we pull towards (momentum_bias - mu)
        // This injects the "Context" into the particle physics.
        // Coupling strength (5.0) determines how much history matters.
        double drift_target = momentum_bias;
        p->mu += 5.0 * (drift_target - p->mu) * dt_eff + 0.5 * sqrt(p->v) * dw_drift * sqrt(dt_eff);
    ...
    if(sum_weight < 1e-100 || isnan(sum_weight)) {
        ...
        for(int i=0; i<N_PARTICLES; i++) {
            swarm[i].weight = 1.0 / N_PARTICLES;
            swarm[i].v = theta_eff;
            // Reset Drift to MOMENTUM, not 0
            swarm[i].mu = momentum_bias; 
        }
    }
