...
// Robust Initialization (Garman-Klass)
...
void optimize_svcj(double* ohlcv, int n, SVCJParams* p, double* out_spot_vol, double* out_jump_prob) {
    ...
    for(int i=0; i<=n_dim; i++) {
        SVCJParams temp = *p;
        if(i==1) temp.kappa *= 1.25; if(i==2) temp.theta *= 1.25; if(i==3) temp.sigma_v *= 1.25;
        if(i==4) temp.rho = (temp.rho > 0) ? temp.rho - 0.25 : temp.rho + 0.25; if(i==5) temp.lambda_j *= 1.5;
