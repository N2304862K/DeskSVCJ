...
cdef extern from "svcj.h":
    ...
    void optimize_svcj_stateful(double* ohlcv, int n, SVCJParams* params, SVCJParams* prev_params, double* out_spot_vol, double* out_jump_prob) nogil
...
def generate_asset_option_adjusted(object ohlcv, double s0, object option_chain):
    ...
    # Cold start (NULL)
    optimize_svcj_stateful(&c_ohlcv[0, 0], n, &p, NULL, &spot_vol[0], &jump_prob[0])
...
def analyze_market_rolling(object market_ohlcv_tensor, int window):
    ...
    with nogil:
        for i in prange(n_assets, schedule='dynamic'):
            # W=0: Cold Start (Pass NULL)
            optimize_svcj_stateful(&data[i, 0, 0], window, &p, NULL, NULL, NULL)
            results[i, 0, 0] = p.theta; results[i, 0, 1] = p.kappa; results[i, 0, 2] = p.sigma_v; results[i, 0, 3] = p.rho; results[i, 0, 4] = p.lambda_j;
            
            # W>0: Warm Start (Pass &p)
            for w in range(1, n_windows):
                optimize_svcj_stateful(&data[i, w, 0], window, &p, &p, NULL, NULL)
                results[i, w, 0] = p.theta; results[i, w, 1] = p.kappa; results[i, w, 2] = p.sigma_v; results[i, w, 3] = p.rho; results[i, w, 4] = p.lambda_j;
    return results
...
def analyze_market_current(object market_ohlcv_tensor):
    ...
    with nogil:
        for i in prange(n_assets):
            # Current snapshot is always a "Cold Start" for this specific run
            optimize_svcj_stateful(&data[i, 0, 0], n_days, &p, NULL, &out_spot[i, 0], &out_jump[i, 0])
...
def generate_residue_analysis(object ohlcv):
    ...
    cdef SVCJParams p
    optimize_svcj_stateful(&c_ohlcv[0, 0], n, &p, NULL, NULL, NULL)
    cdef int t
