...
cdef extern from "svcj.h":
    ...
    ctypedef struct TermStructStats:
        double slope, curvature, short_term_theta, long_term_theta, coherence_score
...
def get_structural_gradient(object ohlcv, double dt):
    ...
    # C-Level Linear Regression
    calculate_structural_gradient(&c_ohlcv[0,0], n, dt, &s)
    
    return {
        "slope": s.slope,
        "short_theta": s.short_term_theta,
        "long_theta": s.long_term_theta,
        "divergence": (s.short_term_theta - s.long_term_theta) / s.long_term_theta
    }
