...
cdef extern from "svcj.h":
    ...
    void optimize_rolling_chain(double* ohlcv_full, int n_total_days, int window, double* out_results) nogil
...
def generate_asset_option_adjusted(object ohlcv, double s0, object option_chain):
    ...
    cdef np.ndarray[double, ndim=1, mode='c'] c_ks = ks; cdef np.ndarray[double, ndim=1, mode='c'] c_ts = ts; cdef np.ndarray[int, ndim=1, mode='c'] c_types = types
    cdef int n_ret = n - 1
...
def analyze_market_rolling(object market_ohlcv_tensor, int window):
    ...
    # 5 params: Theta, Kappa, SigmaV, Rho, Lambda
    cdef np.ndarray[double, ndim=3] results = np.zeros((n_assets, n_windows, 5))
    
    cdef int i
    with nogil:
        for i in prange(n_assets, schedule='dynamic'):
            # Call the new C-level chain optimizer
            # &results[i, 0, 0] gives the pointer to the start of this asset's result block
            optimize_rolling_chain(&data[i, 0, 0], n_days, window, &results[i, 0, 0])
