...
def generate_asset_option_adjusted(object ohlcv, double s0, object option_chain):
    ...
    # N items now (Intraday returns preserve length)
    ...
    price_option_chain(s0, &c_ks[0], &c_ts[0], &c_types[0], n_opts, &p, spot_vol[n-1], &model_prices[0])
...
def generate_residue_analysis(object ohlcv):
    ...
    cdef int t
    for t in range(n):
        residues[t] = np.log(c_ohlcv[t, 3]/c_ohlcv[t, 0]) - (p.mu * (1.0/252.0)); # Intraday Residue
    return residues